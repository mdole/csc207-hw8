...
public class InsertionSorter<T> extends SorterBridge<T> {
   /**
    * Sort vals using insertion sort.  See the Sorter<T> interface
   ...
   /**
    * Insert the value in position i into the sorted subarray
    * in positions 0..(n-1).
    *
    * @param
    ...
    * @param
    *     n, the bound on the end of the subarray.
    *     
    * @pre
    *     0 <= n < values.length
    * @pre
    *     Utils.sorted(values, order, 0, n).
    * @pre
    *     order can be compared to any pair of values in values.
    * @post
    *     Utils.sorted(values, order, 0, n-1)
    * @post
    *     No elements have been added or removed.
    */
   void insert(T[] vals, Comparator<T> order, int n) {
       ...
       while ((i > 0) && (order.compare(vals[i-1],vals[i]) > 0)) {
           ...
           // I3(i-1) hold b/c we know that the value at position i-1 was
           //   the largest.
           // We restore the invariant by subtracting 1 from i.
           i--;
       } // while
       ...
       // is is positive, in which case we know that the left part is
